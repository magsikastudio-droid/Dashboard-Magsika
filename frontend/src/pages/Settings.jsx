...
import { Settings as SettingsIcon, Save, Plus, X, Mail, Phone, Key, Bell, ShieldCheck, Users } from "lucide-react";
...
export default function Settings() {
  const { user } = useAuth();
  const [settings, setSettings] = useState({ allowed_emails: [], fonnte_token: "", admin_wa: "", reminders_enabled: true });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        toast.error("Gagal memuat settings: " + (e?.response?.data?.detail || e.message));
      } finally {
        setLoading(false);
      }
    })();
  ...
  const addEmail = () => {
    ...
    if (settings.allowed_emails.includes(e)) { toast.error("Email sudah ada"); return; }
    setSettings({ ...settings, allowed_emails: [...settings.allowed_emails, e] });
    setNewEmail("");
  };

  const removeEmail = (e) => {
    setSettings({ ...settings, allowed_emails: settings.allowed_emails.filter((x) => x !== e) });
  };
    ...
    <div className="space-y-6 max-w-3xl" data-testid="settings-page">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[var(--ms-text-muted)] text-sm mt-0.5">Akses tim & WhatsApp reminder</p>
        </div>
      </div>
      ...
      <section className="bg-white rounded-2xl border border-[var(--ms-border)] p-6">
        <div className="flex items-center gap-2.5 mb-1">
          <Mail size={16} style={{ color: "var(--ms-primary)" }} />
          <h2 className="font-display text-xl font-bold">Whitelist Email Google</h2>
        </div>
        <p className="text-sm text-[var(--ms-text-muted)] mb-4">Hanya email di daftar ini yang boleh login (admin tetap selalu boleh). Kosongkan untuk mengizinkan semua.</p>
        ...
        <div className="flex gap-2 mb-3">
          ...
          <button onClick={addEmail} className="flex items-center gap-1.5 px-4 rounded-xl text-white text-sm font-semibold transition-base hover:opacity-90 whitespace-nowrap" style={{ background: "var(--ms-primary)" }} data-testid="add-email-btn">
            <Plus size={14} /> Tambah
          </button>
        </div>

        <div className="flex flex-wrap gap-2" data-testid="email-list">
          {settings.allowed_emails.length === 0 && <p className="text-xs text-[var(--ms-text-muted)] italic">Belum ada email — semua orang bisa login.</p>}
          {settings.allowed_emails.map((e) => (
            <span key={e} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium font-mono" style={{ background: "var(--ms-primary-soft)", color: "var(--ms-primary)" }}>
              {e}
              <button onClick={() => removeEmail(e)} className="hover:text-rose-600" data-testid={`remove-email-${e}`}><X size={12} /></button>
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white rounded-2xl border border-[var(--ms-border)] p-6">
        <div className="flex items-center gap-2.5 mb-3">
          <Users size={16} style={{ color: "var(--ms-primary)" }} />
          <h2 className="font-display text-xl font-bold">Tim yang Sudah Login</h2>
        </div>
      ...
      <section className="bg-white rounded-2xl border border-[var(--ms-border)] p-6">
        <div className="flex items-center gap-2.5 mb-1">
          <Bell size={16} style={{ color: "var(--ms-primary)" }} />
          <h2 className="font-display text-xl font-bold">WhatsApp Reminder (Fonnte)</h2>
        </div>
        <p className="text-sm text-[var(--ms-text-muted)] mb-5">Otomatis kirim WA reminder ke nomor admin saat deadline tinggal <strong>3 hari, 2 hari, 1 hari, dan 6 jam</strong>. Pakai <a href="https://fonnte.com" target="_blank" rel="noreferrer" className="underline" style={{ color: "var(--ms-primary)" }}>Fonnte</a> sebagai provider.</p>

        <div className="space-y-4">
          <div>
            <label className={lbl}><Key size={11} className="inline mr-1" /> Fonnte API Token</label>
            <input className={inp} placeholder="Masukkan token Fonnte..." value={settings.fonnte_token} onChange={(e) => setSettings({ ...settings, fonnte_token: e.target.value })} data-testid="fonnte-token-input" />
            <p className="text-xs text-[var(--ms-text-muted)] mt-1.5">Dapat dari dashboard Fonnte → Device → Token.</p>
          </div>
          <div>
            <label className={lbl}><Phone size={11} className="inline mr-1" /> Nomor Admin Tujuan WhatsApp</label>
            <input className={inp} placeholder="6281234567890" value={settings.admin_wa} onChange={(e) => setSettings({ ...settings, admin_wa: e.target.value })} data-testid="admin-wa-input" />
            <p className="text-xs text-[var(--ms-text-muted)] mt-1.5">Format internasional, tanpa <code className="font-mono bg-[var(--ms-bg)] px-1 rounded">+</code>. Contoh: 6281234567890</p>
          </div>
