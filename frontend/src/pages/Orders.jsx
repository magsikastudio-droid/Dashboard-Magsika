...
import { Plus, FileDown, ClipboardList, Search, Trash2, Pencil, AlertTriangle, CheckSquare, Square, Receipt, Archive } from "lucide-react";
              ...
              {filtered.map((o, idx) => {
                ...
                return (
                  <tr key={o.id} className={`border-t border-[var(--ms-border)] hover:bg-[var(--ms-bg)] transition-base ${isDone ? "opacity-60" : ""}`} data-testid={`order-row-${o.id}`}>
                    ...
                    <td className="px-2 py-3 truncate"><Pill label={cap(o.status)} bg={sc.bg} text={sc.text} /></td>
